...
try:
    ...
    HAVE_GMP = False
    print("[NOTICE] Running 10x slower, gotta go fast? pip3 install gmpy2", file=sys.stderr)
...
def decode_challenge(enc):
    dec = enc.split('.')
    if dec[0] != VERSION:
        print("\033[91m [-]Proof-of-work fail\033[0m")
        sys.exit(1)
...
def usage():
    print('Usage:')
    print('Solve pow: python3 verify.py solve $challenge')
    print('Check pow: python3 verify.py ask $difficulty')
    print('$difficulty examples (for 1.6GHz CPU) in fast mode:')
    print('             1337:   1 sec')
    print('             31337:  30 secs')
    print('             313373: 5 mins')
    sys.exit(1)
